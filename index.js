...
const commandDefs = [
  ...
  // --- Store ---
  new SlashCommandBuilder().setName("store").setDescription("Browse the item store"),
  new SlashCommandBuilder().setName("buy").setDescription("Buy an item from the store")
    .addStringOption(o => o.setName("item").setDescription("Item ID to buy").setRequired(true)
      .addChoices(...Object.entries(STORE).map(([k, v]) => ({ name: `${v.emoji} ${v.name} (${v.price} coins)`, value: k })))),
  new SlashCommandBuilder().setName("use").setDescription("Use an item from your inventory")
    .addStringOption(o => o.setName("item").setDescription("Item ID to use").setRequired(true)
      .addChoices(...Object.entries(STORE).map(([k, v]) => ({ name: `${v.emoji} ${v.name}`, value: k }))))
    .addUserOption(o => o.setName("target").setDescription("User to use the item on").setRequired(true)),
  ...
  if (commandName === "help") {
    const embed = new EmbedBuilder().setTitle("📖 Commands").setColor(0x5865f2)
      .addFields(
        ...
        { name: "🎮 Casino",     value: "`/slots` `/blackjack` `/roulette` `/rps` `/dice` `/trivia`" },
